...
let sc = 100; // scale in pixels
let ar = 4/3; // aspect ratio
let c = 0; // # of photos took
let mn = 9; // minimum captures

let mc = document.getElementById("canvas"); // canvas
let vw = mc.getBoundingClientRect().width-8; // video width
...
function draw() {
	...
	vw = mc.getBoundingClientRect().width-8;
	resizeCanvas(vw,vw/ar);
	video.size(vw,vw/ar);

	if (document.getElementById("showCamera").checked) {
		image(video,0,0,vw,vw/ar);
	}
	...
	if (document.getElementById("showKeypoints").checked) {
		for (let i = 0; i < faces.length; i++) {
			...
			for (let j = 0; j < face.keypoints.length; j++) {
				...
				fill(255, 0, 0);
				noStroke();
				circle((keypoint.x-box.xMin)*sc/box.width,(keypoint.y-box.yMin)*sc/box.width, 2);
			}
		}
	}
}
